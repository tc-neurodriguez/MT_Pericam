...
def plot_overlap_results(results: pd.DataFrame, top_n: int = 10):
    ...
    axs[1, 1].barh(top_fluorophores['Fluorophore'],
                   top_fluorophores['TagRFP-T_Overlap_%'],
                   color=colors[3], height=bar_width)
    axs[1, 1].set_title('TagRFP-T Overlap')
...
def main():
    ...
    sensor_configs = [
        ...
        {'filters': {'ex_center': 555, 'em_center': 584}, 'reference': 'TagRFP-T'}  # TagRFP-T
    ]

    sensor_names = ['Pericam_Ca', 'Pericam_pH', 'TagRFP-T']
    ...
    results = analyze_spectral_overlaps(
        ...
        sensor_configs,
        sensor_names,
        exclude=['ratiometric_pericam', 'TagRFP-T']
    )
    # Generate the plots
